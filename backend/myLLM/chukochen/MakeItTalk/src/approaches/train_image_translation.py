...
class Image_translation_block():
    ...
    def __init__(self, opt_parser=None, single_test=True,comb_G=None):
        ...
        # model
        if comb_G is not None:
            self.G = comb_G
        elif (opt_parser.load_G_name != ''):
    ...
    def single_test(self, jpg=None, fls=None, filename=None, grey_only=False):
        self.G.eval()
        ...
        output_file_path = f"{time.time()}.mp4"
        # 输出当前所在位置
        os.system(f'pwd')
        os.system(f'ffmpeg -loglevel error -y -i {"./temp/" + next_file} -i {"./media/audio/" + filename[9:-16] + ".wav"} -pix_fmt yuv420p -strict -2 {"./media/video/" + output_file_path}')
        # os.system('rm out.mp4')
        return output_file_path

    def translation(self,img,scale,shift,filenames):
        outputnames = []
        for i in range(0,len(filenames)):
            fl = np.loadtxt(os.path.join('./temp', filenames[i])).reshape((-1, 68,3))
            fl[:, :, 0:2] = -fl[:, :, 0:2]
            fl[:, :, 0:2] = fl[:, :, 0:2] / scale - shift
            fl = Image_translation_block.add_naive_eye(fl)
            # additional smooth
            fl = fl.reshape((-1, 204))
            ...
            with torch.no_grad():
                outputnames.append(self.single_test(jpg=img, fls=fl, filename=filenames[i]))
            os.remove(os.path.join('./temp', filenames[i]))
        return outputnames
