...
class Audio2landmark_model():
    ...
    def __init__(self, opt_parser=None,
                 ...
                 a2l_C=None,
                 test_emb=None,
                 eval_data:Audio2landmark_Dataset =None
                 ):
        '''
        ...
        # else:
            # self.eval_data = Audio2landmark_Dataset(dump_dir='./dump',
                                                # dump_name='random',
                                                # status='val',
                                            #    num_window_frames=18,
                                            #    num_window_step=1,
                                            #    au_mean_std=au_mean_std)
                # Step 3: Load model
        if a2l_G is not None:
            self.G = a2l_G
        else:
        ...
        # self.anchor_t_shape = anchor_t_shape
        # self.anchor_t_shape = self.anchor_t_shape[self.t_shape_idx, :]
        if test_emb is not None:
            self.test_embs = test_emb

    def update_test_emb(self,test_emb):
        self.test_embs = test_emb
    def update_face(self,jpg_shape):
        self.std_face_id = jpg_shape
        self.std_face_id = self.std_face_id.reshape(1, 204)
        self.std_face_id = torch.tensor(self.std_face_id, requires_grad=False, dtype=torch.float).to(device)
    def set_eval_data(self,eval_data:Audio2landmark_Dataset):
        self.eval_data = eval_data
        self.eval_dataloader = torch.utils.data.DataLoader(self.eval_data, batch_size=1,
                                                           shuffle=False, num_workers=0,
                                                           collate_fn=self.eval_data.my_collate_in_segments)

    def __train_face_and_pos__(self, fls, aus, embs, face_id, smooth_win=31, close_mouth_ratio=.99,**kwargs):
    ...
    #
        ...
        ''' ============================================================= '''
        amp_pos = kwargs.get("amp_pos",.5)
        fl_dis_pred = torch.tensor(fl_dis_pred).to(device) * amp_pos

        residual_face_id = baseline_face_id

        # ''' CALIBRATION '''
        baseline_pred_fls, _ = self.C(aus[:, 0:18, :], residual_face_id)
        baseline_pred_fls = self.__calib_baseline_pred_fls__(baseline_pred_fls,kwargs=kwargs)
        fl_dis_pred += baseline_pred_fls

        return fl_dis_pred, face_id[0:1, :]

    def __calib_baseline_pred_fls_old_(self, baseline_pred_fls, residual_face_id, aus,**kwargs):
        ...
        amp_lip_x = kwargs.get("amp_lip_x",2.0)
        amp_lip_y = kwargs.get("amp_lip_y",2.0)
        baseline_pred_fls[:, 48 * 3::3] *= amp_lip_x  # mouth x
        baseline_pred_fls[:, 48 * 3 + 1::3] *= amp_lip_y  # mouth y
        return baseline_pred_fls

    def __calib_baseline_pred_fls__(self, baseline_pred_fls, ratio=0.5,**kwargs):
    ...
    def __train_pass__(self, au_emb=None, centerize_face=False, no_y_rotation=False, vis_fls=False,**kwargs):
        ...
        for i, batch in enumerate(dataloader):
            ...
            for key in keys: # ['45hn7-LXDX8']: #['sxCbrYjBsGA']:#
                ...
                np.savetxt(os.path.join("./temp", filename), fake_fls_np, fmt='%.6f')
    ...
    def test(self, au_emb=None,**kwargs):
        with torch.no_grad():
            return self.__train_pass__(au_emb, vis_fls=False,kwargs=kwargs)
