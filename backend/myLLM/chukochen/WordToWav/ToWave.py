...
class Ws_Param(object):
    ...
    def generate_wav(self, audio_dir):
        current_dir = os.path.dirname(__file__)
        parent_dir = os.path.abspath(os.path.join(current_dir, os.pardir))
        audio_dir = os.path.join(parent_dir, audio_dir)
        result = []
        websocket.enableTrace(False)
        wsUrl = self.create_url()
        ws = websocket.WebSocketApp(wsUrl, on_message=get_total_message(result), on_error=on_error, on_close=on_close)
        ws.on_open = on_open_param(self, audio_dir)
...
def on_open_param(wsParam, audio_dir):
    def on_open(ws):
        def run(*args):
